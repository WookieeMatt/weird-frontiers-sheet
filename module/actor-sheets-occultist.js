/**
 * WF Occultist character sheet overrides
 */

import DCCActorSheet from '/systems/dcc/module/actor-sheet.js'

/**
 * Extend the zero-level/NPC sheet for WF Occultist
 * @extends {DCCActorSheet}
 */
class ActorSheetOccultist extends DCCActorSheet {
  /** @override */
/**  getData () {
    const data = super.getData() **/
async getData (options) {
    const data = await super.getData(options)
    this.options.template = 'modules/weird-frontiers-sheet/templates/actor-sheet-occultist.html'
    if (data.system.details.sheetClass !== 'occultist') {
      this.actor.update({
        'data.class.className': game.i18n.localize('WF.occultist')
      })
    }
